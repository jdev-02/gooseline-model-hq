...
- `data/mlb/narrative/YYYY-MM-DD.yaml` + `log.csv` — human tilts and every
  ...
  the game. Since 2026-09-15 each row also carries `tier` (`bet` / `small` /
  `pass` / `stale`) and `label` (the exact words on the card: `BET · Over 9.5
  runs`, `SMALL BET · Over 9.5 runs`, `PASS`, `PRICE STALE · re-check`),
  decided by `src/mlb/labels.py` from the live paper-trade record. BET is
